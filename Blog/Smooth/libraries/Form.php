<?php 
	...
	class Form
	{
		...
		public static function filter($field, $rules = null)
		{
			...
			{
				Handler::handler('You must set your field name or rules');
			}

			if( !is_string($field) or !is_string($rules) )
			{
				Handler::handler('Strings only accepted for ' . $filed . ' and ' . $rules . ' arguments.');
			}
			...
			$rule = explode('.', $rules);
				...
				foreach($rule as $separate_rule)
				{
					...
					{
						Handler::handler('Not callable function');
					}
				}
		}
		 ...
		 * @return [type]
		 */
		public static function length($field, $min_length, $max_length)
		{
			if( ! is_int($min_length) or ! is_int($max_length) )
			{
				Handler::handler('Not valid arguments');
			}
			if( strlen($_POST[$field]) < $min_length )
				Handler::handler('Does not meet the minimum legth');
			elseif ( strlen($_POST[$field]) > $max_length)
				Handler::handler('Does not meet hte maximum length');
		}
		...
		public static function alpha_numeric_dash_under($string)
		{
			return ( ! preg_match('/^[a-zA-Zа-яА-Я\-\_]+$/', $string) ) ? false : true;
		}
		...
		{
			return '<form method="' . strtoupper($method) . '" action="' . $action . '">';
		}
		public static function input($type, $name, $value='', $placeholder='', $pattern=null, $required=null, $title=null, $old='')
		{
		...
		public static function ul($name, $id = '', $class = '')
		{
